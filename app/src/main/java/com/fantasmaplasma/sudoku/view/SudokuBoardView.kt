package com.fantasmaplasma.sudoku.view
...
import com.fantasmaplasma.sudoku.util.Constant.SQRT_SIZE
...
class SudokuBoardView(context: Context, attributeSet: AttributeSet) : View(context, attributeSet) {
    ...
    private val noteBackgroundPaint = Paint().apply {
        style = Paint.Style.FILL_AND_STROKE
    ...
    private var animationRow2 = 0
    private var animationCol2 = 0
    private var animationRow = 0
    private var animationCol = 0
    private var spiralTraverse = true
    ...
    private fun fillCells(canvas: Canvas) {
        ...
        val cellSelectedIsStartingCell =
            getCell(selectedRow, selectedCol)?.isStartingCell ?: false
        ...
        cells?.forEach { cell ->
            ...
            if (r == selectedRow && c == selectedCol) {
                val paint =
                    ...
                    else if(cell.value < 1 && cell.notes.isNotEmpty())
                        noteBackgroundPaint
                    else
                        startingCellPaint
                fillCell(canvas, r, c, paint)
                if(paint == startingCellPaint)
                    drawSelectedSquare(canvas, r, c)
            } else if(cellSelectedIsStartingCell && cell.isStartingCell) {
                fillCell(canvas, r, c, conflictingValidCellPaint)
            } else if (r == selectedRow || c == selectedCol ||
                (r / SQRT_SIZE == selectedRow / SQRT_SIZE && c / SQRT_SIZE == selectedCol / SQRT_SIZE)) {
                val paint =
                    if(cell.value < 1 && cell.notes.isNotEmpty())
                        noteBackgroundPaint
                    else
                        conflictingCellPaint
                fillCell(canvas, r, c, paint)
            } else if(cell.value < 1 && cell.notes.isNotEmpty()) {
                fillCell(canvas, r, c, noteBackgroundPaint)
            }
        }
    }
